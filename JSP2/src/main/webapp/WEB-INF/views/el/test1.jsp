...
<pre>
 ...
 - \${paramValues.key} : key 일치하는 파라미터 모두 얻어오기
	...
	<%-- HTML에서 얻어온 데이터는 모두 String
		하지만 EL 에서 연산되는 자료형이 다를경우
		자동으로 자료 형변환 진행해서 비교해준다 --%>
	...
	<ul>
	<%-- check가 여러개 전달된 경우 첫번째 파라미터 값 출력 --%>
	<li>param.check:${param.check}</li>
	<li>
	<%-- check 라는 key 를 가진 모든 파라미터 값을 모아 String[]로 반환 --%>
	parmaValues.check : ${paramValues.check} <%-- 배열의 주소 --%>
	</li>

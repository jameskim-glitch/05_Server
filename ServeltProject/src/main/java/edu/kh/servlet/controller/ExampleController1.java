 ...
 * 
 * 2. @wevServlet() 어노테이션 이용 방법
 * 
 * */

// "/example" 이라는 클라이언트의 요청을 받아서 처리 한 후 응답을 해줄
// 서블릿 클래스 ExampleController 를 만든다
...
public class ExampleController1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// HttpServletResponse
		// - 클라이언트 요청 시 생성되는 객체
		// - 서버가 클라이언트 에게 응답하기 위한 방법을 제공함.
		// 서버 -> 클라이언트 로 연결된 출력용 스트림 제공(PrintWriter)
		
		System.out.println("--- 이름, 나이를 입력받아 처리하는 서블릿 코드 ---");
		
		// 요청 시 입력된 이름과 나이를 전달받아 오기
		...
		sb.append("<html>");
		sb.append("<head>");
		sb.append("<title>서버 응답 패이지</title>");	
		sb.append("</head>");	

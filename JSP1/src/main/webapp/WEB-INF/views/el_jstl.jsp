...
<pre>
	<%-- 
			<%= %>
	 --%>
 - JSP 에서 표현식을 간단하고 효율적으로 작성할 수 있도록 고안된 언어
 (JSP 내부에 기본 내장되어 있음!)
...
<h4>2. EL </h4>
	<%-- 
	EL 특징 :  1. null, NullPointException 을 빈 칸으로 처리
...
<pre>
	...
	(if, for, 변수선언...)
	스크립틋릿이 아닌 HTML 태그 모양으로 작성할수 있도록
	태그를 제공하는 라이브러리
	
	[라이브러리 추가 방법]
	1. 필요한 라이버트리(.jar) 파일을 다운 받기
	2. 프로젝트 webapp/WEB-INF/lib 폴더에
	다운로드 받은 라이브러리 추가(드래그앤드랍, 복붙)
	3. JSTL 을 사용할 경우 JSP 파일 제일 위에
	taglib 추가 구문 작성
</pre>
...
<c:if test="${param.age <= 20 }"><h3>성인이 아닙니다(JSTL사용).</h3></c:if>
